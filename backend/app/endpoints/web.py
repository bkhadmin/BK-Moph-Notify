...
def templates_page(request:Request, edit_id:int|None=None, db:Session=Depends(get_db)):
    ...
    return templates.TemplateResponse('admin/templates.html', ctx(request, db, session, message_templates=get_templates(db), render_result=None, media_files=get_media(db), notify_rooms=get_active_notify_rooms(db), notify_rooms=get_active_notify_rooms(db), flex_sample=flex_sample, edit_row=edit_row))
...
def render_template_page(request:Request, content:str=Form(...), variables_json:str=Form('{}'), db:Session=Depends(get_db)):
    ...
    flex_sample = '{"type":"bubble","body":{"type":"box","layout":"vertical","contents":[{"type":"text","text":"สวัสดี {name}"},{"type":"text","text":"หน่วยงาน {organization_name}","size":"sm"}]}}'
    return templates.TemplateResponse('admin/templates.html', ctx(request, db, session, message_templates=get_templates(db), render_result=render_result, media_files=get_media(db), notify_rooms=get_active_notify_rooms(db), notify_rooms=get_active_notify_rooms(db), flex_sample=flex_sample, edit_row=None))
...
def notify_flex_builder(
    ...
    return templates.TemplateResponse('admin/notify_test.html', ctx(
        ...
        media_files=get_media(db), notify_rooms=get_active_notify_rooms(db), notify_rooms=get_active_notify_rooms(db), flex_json=flex_json
    ))
...
async def notify_send_minimal_flex(request:Request, db:Session=Depends(get_db)):
    ...
    try:
        ...
        return templates.TemplateResponse('admin/notify_test.html', ctx(
            ...
            approved_queries=get_queries(db), message_templates=get_templates(db),
            media_files=get_media(db), notify_rooms=get_active_notify_rooms(db), notify_rooms=get_active_notify_rooms(db), flex_json=json.dumps(payload[0]["contents"], ensure_ascii=False, indent=2)
        ))
    except Exception as exc:
...
def notify_preview(request:Request, approved_query_id:int=Form(...), message_template_id:int=Form(...), notify_room_id:int|None=Form(None), db:Session=Depends(get_db)):
    ...
    try:
        ...
        return templates.TemplateResponse('admin/notify_test.html', ctx(
            ...
            approved_queries=get_queries(db), message_templates=get_templates(db),
            media_files=get_media(db), notify_rooms=get_active_notify_rooms(db), notify_rooms=get_active_notify_rooms(db), flex_json=None
        ), status_code=200)
...
async def notify_send_from_template(request:Request, approved_query_id:int=Form(...), message_template_id:int=Form(...), notify_room_id:int|None=Form(None), db:Session=Depends(get_db)):
    ...
    try:
        ...
        return templates.TemplateResponse('admin/notify_test.html', ctx(
            ...
            approved_queries=get_queries(db), message_templates=get_templates(db),
            media_files=get_media(db), notify_rooms=get_active_notify_rooms(db), notify_rooms=get_active_notify_rooms(db), flex_json=json.dumps(messages[0]["contents"], ensure_ascii=False, indent=2) if isinstance(messages, list) and messages and messages[0].get("type") == "flex" else None
        ))
    except Exception as exc:
